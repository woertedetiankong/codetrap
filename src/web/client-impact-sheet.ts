...
export function createImpactSheet(dialog: HTMLDialogElement, reducedMotion: () => boolean) {
  let position = 0, velocity = 0, target = 0, frame = 0, last = 0;
  let closing = false, opener: HTMLElement | null = null;
  let dragging: {id:number; axis:"clientX"|"clientY";start:number;origin:number;samples:Array<[number,number]>} | null = null;
  ...
  function settle(next: number, close = false) {
  ...
  function show(title: string, html: string) {
    ...
    (dialog.querySelector<HTMLElement>('.sheet-content input:not([type=checkbox])') || dialog.querySelector<HTMLElement>('button'))?.focus({preventScroll:true});
  }
  ...
  const handle = dialog.querySelector<HTMLElement>('[data-sheet-drag]')!;
