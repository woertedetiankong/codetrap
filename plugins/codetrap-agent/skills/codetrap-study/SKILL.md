---
name: codetrap-study
description: Turn a blog, documentation, code, or an existing codetrap Learning insight into a self-contained interactive HTML lesson with SVG demonstrations, then attach or update it in the local Learning library when the user asks to save it.
---
...
Do not invent IDs. If the request creates new study content, use the bundled
`codetrap-capture-external` workflow for a source-backed collection, or
`codetrap-learning-review` only when the user explicitly requests session review.
These are present in the same plugin. Preserve source coverage and the ordinary
