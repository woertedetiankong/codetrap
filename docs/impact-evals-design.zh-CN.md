...
## 2026-09-08：整个 Impact 已替换为已确认原型

正式界面已整体替换为概览、任务记录、改进工作台和验证四个区域。任务提供只读经验抽屉与轻量反馈，修订进入三步工作台；验证区区分经验修订与检索对照。旧三栏布局及旧总览、轨迹、评测页面模板已经移除。

后端增加项目范围的改进投影，聚合明确反馈和漏召回问题，关联已保存修订；经验正文只在版本吻合时显示。查询缺失、未收到结束事件、尚无后续证据均明确显示。原有反馈幂等、草稿恢复、验证摘要校验、精确版本应用与回滚规则继续有效。

- [当前修订与证据契约](experience-revisions.md)
- [检索对照与用例操作](project-evaluation-suites.md)
- [整体迁移与验证记录](tasks/2026-09-08-impact-production-migration/handoff.md)
- [改版前审查与开源参考](research/2026-09-07-evals-ux/README.md)
