...
- 2026-09-05 Learning drafts use bounded browser-local backups with explicit restoration. Immutable snapshot keys preserve concurrent-tab versions, and recovery does not submit data or restore validation/approval. Review/Evals durable recovery remains a separate slice. See the [draft recovery handoff](tasks/2026-09-05-learning-draft-recovery/handoff.md).

- 2026-09-05 Learning workflow state now uses raw, versioned source-project/Insight drafts. Preview validates a submitted version without rewriting visible fields; submission stages an explicitly reviewed Candidate Inbox item. Durable recovery is implemented in the later slice; collection/filter presentation, Run choices and broader Impact typing remain open. See the [Learning workflow handoff](tasks/2026-09-05-learning-workflow/handoff.md).
