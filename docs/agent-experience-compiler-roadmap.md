...
| Learning workflow state and drafts | **Done** | 2026-09-05 | [handoff](tasks/2026-09-05-learning-workflow/handoff.md) | Source-scoped practice/proposal drafts, captured writes, preview version protection, guarded reads and authorization recovery pass 630 tests. Checkpointed as `ccc317b` at the user's request. |
| Learning durable draft recovery | **Done** | 2026-09-05 | [handoff](tasks/2026-09-05-learning-draft-recovery/handoff.md) | Explicit restore/delete, independent concurrent snapshots, 30-day expiry and storage warnings pass 648 tests; checkpointed after `ccc317b` at the user's request. |
| Five-stage product optimization | **Done** | 2026-09-05 | [handoff](tasks/2026-09-05-five-stage-product-polish/handoff.md) | Review/Evals recovery, typed catalog/Impact requests, optional learning guidance, measured history caching and one real-task evidence chain. 662 tests, strict checks and compiled browser delivery pass. Baseline `f763195`; recorded in `feat: complete five-stage product polish`. |

### Closed decisions by date

- 2026-09-05 Learning drafts use bounded browser-local backups with explicit restoration. Immutable snapshot keys preserve concurrent-tab versions, and recovery does not submit data or restore validation/approval. Review/Evals recovery is implemented in the subsequent five-stage slice. See the [draft recovery handoff](tasks/2026-09-05-learning-draft-recovery/handoff.md).

- 2026-09-05 Learning workflow state now uses raw, versioned source-project/Insight drafts. Preview validates a submitted version without rewriting visible fields; submission stages an explicitly reviewed Candidate Inbox item. Durable recovery is implemented in the later slice; collection/filter presentation remains a legacy boundary; Run choices and Impact business state are typed in the subsequent five-stage slice. See the [Learning workflow handoff](tasks/2026-09-05-learning-workflow/handoff.md).
...
- 2026-09-05 Review state, drafts and mutation coordination now live in strict browser modules. Drafts are per project/session/candidate; the subsequent five-stage slice adds browser backup recovery, while writes capture the original identity and visible fields. Destination-specific renderers remain in the workspace, and backend cross-process edit concurrency is unchanged. The later Learning workflow slice builds on this boundary; see the [Review handoff](tasks/2026-09-04-review-state/handoff.md).
