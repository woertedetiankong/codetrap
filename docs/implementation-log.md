...
- Task: finish remaining useful simplification work from `docs/simplification-plan.zh-CN.md` under `goal-brief-remaining-simplification.md`.
- Preflight codetrap search returned trap #3 for search/list applicability and trap #5 for SQLite startup/migration locking. I will avoid changing search applicability semantics and will keep schema edits limited to migration table-shape branching, without touching connection startup ordering.
- Revalidation found several original simplification claims are stale: `scope-path`, `trap-json-fields`, `search-result-card`, `trap-lifecycle`, `trap-mutation-result`, and related modules still have tests, docs, or shared adapter value. The goal will prefer documenting those decisions over deleting named boundaries for file-count reduction.
- Final simplification pass removed the `embedding-index` delegate, simplified `migrateEmbeddingProfiles`, removed unused re-exports, shared repeated low-level helpers, and documented every rejected/deferred plan item in `docs/simplification-remaining-status.zh-CN.md`. Validation passed with `bunx tsc --noEmit`, full `bun test src/tests`, `git diff --check`, and `bun run eval:dogfood -- report`.
