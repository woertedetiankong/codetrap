...
下面以 `0.1.2` 为例。每次发布前把版本号换成你要发布的新版本。
...
- npm 同一个版本号只能发布一次。
- Git tag 也不要重复使用。
- `package.json` 版本 `0.1.2` 对应 tag `v0.1.2`。
...
npm install -g --prefix /tmp/codetrap-npm-test /tmp/codetrap-pack-test/codetrap-0.1.2.tgz
...
BUN_INSTALL=/tmp/codetrap-bun-test bun add -g /tmp/codetrap-pack-test/codetrap-0.1.2.tgz
...
```bash
gh release view v0.1.2 --repo woertedetiankong/codetrap --json url,tagName,assets
```
...
```bash
gh workflow run npm-publish.yml --repo woertedetiankong/codetrap --ref v0.1.2 -f tag=v0.1.2
```
...
不要复用 tag。换一个新版本，比如从 `v0.1.2` 升到 `v0.1.3`。
